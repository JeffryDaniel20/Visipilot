...
- [ ] Vertical-slice pass criteria met (see `implementation-plan.md` Phase A) — 20-run batch, ≥18/20 success, recorded with the exact command and output artifact. **Not yet run at the required scale** — a single real traced run succeeds completely (`tests/test_tracing.py::test_full_vertical_slice_instruction_traced_end_to_end`), which is real, necessary evidence, but the actual 20-run pass gate needs the `visipilot/eval/vertical_slice.py` CLI harness, which doesn't exist yet (see implementation-plan.md's "Exact vertical-slice evaluation command" section — this is the natural next step).
