...
- `visipilot/target_selection/` — instruction parsing (`instruction_parser.py`, rule-based FIND/TYPE/CLICK clause splitting) and the matcher (`matcher.py::match_target()`, text + relation + structural + aspect-ratio scoring).
...
- 98/98 tests passing (`pytest`, ~64s), including real trusted clicks/types against a live browser and real pixels-first verification of their effects. Combined detector+OCR peak VRAM measured at **1.71 GB**; a full screenshot→perception→semantic→find→type→verify chain completes in **2.79 s**, both comfortably within budget.
- Honest findings worth flagging (measured, not smoothed over): OWLv2's zero-shot type classification is weak on this flat synthetic test page. Real integration testing caught a genuine matcher bug (a distractor paragraph's incidental text overlap out-scored the real search input — fixed, with a regression test) and, this milestone, partially closed a real ambiguity with an aspect-ratio heuristic (wide elements read as inputs, not buttons) — which resolves the FIND step but **not** the CLICK step's bare "Search" tie. Running the actual vertical-slice instruction for real confirms this: FIND and TYPE succeed, CLICK correctly refuses to guess between the tied input and button. The vertical-slice pass criteria are therefore **not yet met** — this is real, open work, not a formality.

Not yet implemented: tracing — see `implementation-plan.md` Phase A for the remaining checklist.
